...
// → approve names → watch live status and class accuracy BY REGION → download
// the PDF → End Session (deletes everything from memory). Build-a-Colony is
// variant solo with no branch, so students start on their own as soon as
// they're approved — no pairing needed.
...
const REGION_ORDER = ['newengland', 'middle', 'southern'];
...
export default function CommandCenter() {
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Build-a-Colony · New England, Middle, or Southern · TEKS 8.2B, 8.10B, 8.11A, 8.12A, 8.31B</p>
  ...
  const regionCls = REGION_ORDER.map((r) => [r, roster?.classAccuracy?.[r] || { count: 0, average: 0 }]);
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Build-a-Colony · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student begins their colony’s story right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Student</th><th>Region</th><th>Status</th><th>Accuracy</th><th></th></tr>
            </thead>
            <tbody>
              {students.length === 0 && (
      ...
      <div className="card">
        <h2>Class accuracy by region</h2>
        <div className="class-summary">
          {regionCls.map(([r, cls]) => (
            <div key={r} className="summary-stat">
              <div className="big-number">{cls.count ? `${cls.average}%` : '—'}</div>
              <div className="muted">{REGION_LABEL[r]} · {cls.count} completed</div>
            </div>
          ))}
        </div>
      ...
      <footer className="app-footer">8th Grade U.S. History · Build-a-Colony · TEKS 8.2B, 8.10B, 8.11A, 8.12A, 8.31B</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  doc.setFontSize(16);
  doc.text('Build-a-Colony — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  autoTable(doc, {
    startY: 30,
    head: [['Student', 'Region', 'Status', 'Accuracy']],
    body: rows,
    styles: { fontSize: 9 },
  ...
  autoTable(doc, {
    ...
    head: [['Class accuracy by region', '']],
    body: regionCls.flatMap(([r, cls]) => [
      [`${REGION_LABEL[r]} — completed`, String(cls.count)],
      [`${REGION_LABEL[r]} — average accuracy`, cls.count ? `${cls.average}%` : '—'],
    ]),
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('8th Grade U.S. History · Build-a-Colony · TEKS 8.2B, 8.10B, 8.11A, 8.12A, 8.31B', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`build-a-colony_${roster.joinCode}_${date}.pdf`);
}
